...
declare function tx:list-references($node as node(), $poem as node()) as node()* {
  ...
  let $lineRefs := for $val in distinct-values($nameRefs/ancestor::tei:l[1]/@n)
  return
    xs:integer($val)
  let $genderRefs := $poem//tei:ref[some $c in tokenize(@corresp, '\s+')
    satisfies $c = $idRef]
          ...
          <tr>
            <td>
              <a href="view.html?id={$poem/@xml:id}">
                {string-join($poem//tei:head/descendant::text(), '')}
              </a>
            </td>
            <td>
              {
                if (exists($poem[matches(@xml:id, '^b')])) then
                  'Byron'
                else
                  'Coleridge'
              }
